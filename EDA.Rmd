...
```{r echo=FALSE}
store <- c(min(df_eff$Time[df_eff$Brand=='store']),
           mean(df_eff$Time[df_eff$Brand=='store']),
           max(df_eff$Time[df_eff$Brand=='store']),
           max(df_eff$Time[df_eff$Brand=='store'])-min(df_eff$Time[df_eff$Brand=='store']))

name <- c(min(df_eff$Time[df_eff$Brand=='name']),
          mean(df_eff$Time[df_eff$Brand=='name']),
          max(df_eff$Time[df_eff$Brand=='name']),
          max(df_eff$Time[df_eff$Brand=='name'])-min(df_eff$Time[df_eff$Brand=='name']))

temp6 <- c(min(df_eff$Time[df_eff$Temp==6]),
           mean(df_eff$Time[df_eff$Temp==6]),
           max(df_eff$Time[df_eff$Temp==6]),
           max(df_eff$Time[df_eff$Temp==6])-min(df_eff$Time[df_eff$Temp==6]))

temp23 <- c(min(df_eff$Time[df_eff$Temp==23]),
            mean(df_eff$Time[df_eff$Temp==23]),
            max(df_eff$Time[df_eff$Temp==23]),
            max(df_eff$Time[df_eff$Temp==23])-min(df_eff$Time[df_eff$Temp==23]))

temp40 <- c(min(df_eff$Time[df_eff$Temp==40]),
            mean(df_eff$Time[df_eff$Temp==40]),
            max(df_eff$Time[df_eff$Temp==40]),
            max(df_eff$Time[df_eff$Temp==40])-min(df_eff$Time[df_eff$Temp==40]))

stiry <- c(min(df_eff$Time[df_eff$Stirred=='yes']),
           mean(df_eff$Time[df_eff$Stirred=='yes']),
           max(df_eff$Time[df_eff$Stirred=='yes']),
           max(df_eff$Time[df_eff$Stirred=='yes'])-min(df_eff$Time[df_eff$Stirred=='yes']))

stirn <- c(min(df_eff$Time[df_eff$Stirred=='no']),
           mean(df_eff$Time[df_eff$Stirred=='no']),
           max(df_eff$Time[df_eff$Stirred=='no']),
           max(df_eff$Time[df_eff$Stirred=='no'])-min(df_eff$Time[df_eff$Stirred=='no']))


dfrange <- data.frame(store,name,temp6,temp23,temp40,stiry,stirn)
dfrange <- t(dfrange)
colnames(dfrange) <- c("Low", "Mean", "High", "Range")
rownames(dfrange) <- c("Store","Name","Temperature 6","Temperature 23","Temperature 40","Stirred","Not Stirred")
round(dfrange,4)
```
